...
(* a rajouter donner pas dieu nicolas gamestate : t_gamestate ref ;
      game_speed : int ref;*)
    (************************ VECTOR PART ***************************************************************************)
    (**
      ...
      (
        {
          ...
          ball_list  = ref [make_ball(395,650,5)];
          
           }
      )
    ;;
    (*
    truc garder au cas ou a degager quand fonction fini <3<3 
          ball_list  = ref [make_ball(395,650,5)]; 
    {ball_size = ref 5 ; ball_color = GRAY ; ball_coordonates= ref ({dx = 0 ; dy = 0}) ; ball_velocity = ref ({dx = 0 ; dy = 0}); ball_scalar = ref 3};
      *)  
    type t_ball = 
      {
      ball_size : int ref ; 
